...
**Pre-release:** packages ship as `0.9.0` (see `Directory.Build.props` for `VersionPrefix`).
...
| `Microsoft.OpenAPI.FunctionalExtensions.Linting/` | **Linting** (`Functional.Microsoft.OpenAPI.Extensions.Linting`) — configurable lint rules, example validation |
...
Published NuGet packages (4):

| PackageId | Project |
|-----------|---------|
| `Functional.Microsoft.OpenAPI.Extensions` | Core |
| `Functional.Microsoft.OpenAPI.Extensions.Linting` | Linting |
| `Functional.Microsoft.OpenAPI.Extensions.Visualizing` | Visualizing |
| `Functional.Microsoft.OpenAPI.Extensions.Tool` | CLI tool |

Linting is a **separate NuGet package** — consumers reference `Functional.Microsoft.OpenAPI.Extensions.Linting` independently of Core when they only need validation, or alongside Core/Visualizing for full workflows.

Layering (data collection is separate from rendering):

```
AdapterCore → ActivePatterns → *Adapters → Traversal (SchemaGraph / RouteMap / LinksGraph IR)
    → Merge / Scissors / WriterTools → Linting (LintResult)
    → Visualizing (Graphviz) → DOT / SVG
```
...
| Change | Update |
|--------|--------|
| New lint rules | `docs/LINTING.md` — rule ID, description, severity; also `docs/skills/openapi-fsharp-extensions/SKILL.md` and `REFERENCE.md` |
| New CLI commands/flags | `README.md` CLI section; `docs/skills/openapi-fsharp-extensions/SKILL.md` CLI table |
...
Local development requires .NET 10 SDK. SVG rendering additionally requires [Graphviz](https://graphviz.org/) (`dot` in `PATH`).
