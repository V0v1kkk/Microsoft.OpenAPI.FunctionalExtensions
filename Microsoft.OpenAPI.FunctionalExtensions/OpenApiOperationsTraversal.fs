...
let private schemaRefPointer (schema: OpenApiSchema) : string option =
  match schema with
  | null -> None
  | s when String.IsNullOrWhiteSpace s.Id -> None
  | s -> Some ($"#/components/schemas/{s.Id}")

let private collectSchemasFromContent (content: IDictionary<string, OpenApiMediaType>) =
  match content with
  | null -> Seq.empty
  | c ->
      c.Values
      |> Seq.choose (fun mt ->
          ...
          | m ->
              match m.Schema with
              | :? OpenApiSchema as s -> schemaRefPointer s
              | _ -> None)

let private collectParameterSchemas (parameters: IList<IOpenApiParameter>) =
  ...
  | ps ->
      ...
      |> Seq.choose (fun p ->
          match p with
          | null -> None
          | p when not (isNull p.Schema) ->
              match p.Schema with
              | :? OpenApiSchema as s -> schemaRefPointer s
              | _ -> None
          | p when not (isNull p.Content) -> collectSchemasFromContent p.Content |> Seq.tryHead
          | _ -> None)

let private collectRequestSchemas (request: IOpenApiRequestBody) =
  match request with
  | null -> Seq.empty
  | r -> collectSchemasFromContent r.Content

let private collectResponseSchemas (responses: OpenApiResponses) =
...
let collectRouteMap (doc: OpenApiDocument) : RouteMap =
  ...
  if not (isNull doc.Paths) then
    for pathKv in doc.Paths do
      let path = pathKv.Key
      let item = pathKv.Value
      if not (isNull item) && not (isNull item.Operations) then
        for opKv in item.Operations do
          let opType = opKv.Key
          let op = opKv.Value
